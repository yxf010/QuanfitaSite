var Auth = {
	...
	login(e) {
		...
		Auth.vars.lowin_login.classList.remove('lowin-flip');
	...
	init(option) {
		...
		if(option.tag == 'login' || option.tag == ''){
			Auth.vars.box[1].className += ' lowin-flip';
			Auth.setHeight(Auth.vars.box[0].offsetHeight + Auth.vars.lowin_footer.offsetHeight);
		}
		else{
			Auth.vars.box[0].className += ' lowin-flip';
			Auth.setHeight(Auth.vars.box[1].offsetHeight + Auth.vars.lowin_footer.offsetHeight);
		}
